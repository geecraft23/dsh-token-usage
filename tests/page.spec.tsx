...
it('keeps deployment and input/output independent and filters exact provider-model routes', async () => {
  ...
  await screen.findByText('按部署位置')
  expect(screen.getAllByText('0.22K').length).toBeGreaterThan(0)
  expect(screen.queryByText('0.11K')).toBeNull()
...
it('keeps inactive dates and grades active dates against the selected peak', async () => {
  const ledger = new Ledger(':memory:', [], Date.now())
