...
活动图默认显示近 90 天，可切换近 30 天，每天都有一个格子；没有请求记录的日期为浅灰白色，有活动的日期按当前筛选范围的峰值分为四档蓝色。Token 默认以 K 显示，达到百万用 M、十亿用 B；悬停查看精确数值，JSON 导出保留完整整数。
[English](README.md) · 简体中文

**在 DSH 里看清每个模型用了多少 token，以及本地模型一共输出了多少。**

这是 DeepSeek Harness 的独立插件。在 **设置 → Token 用量** 中查看跨会话累计、每日趋势，以及按服务和模型划分的云端／本地用量。
...
历史记录与已保存的分类继续保留，无需迁移账本。旧配置 `online` / `offline` 仍可使用，新配置支持 `cloud` / `local`。JSON 导出升级为 `formatVersion: 2`，来源字段使用 `cloud` / `local`。
